...
$( document ).ready(function() {
  ...
  timer_length = $('.reserver-cart #time').length;
  if (timer_length>0){
    var display = document.querySelector('#time');
    startTimer(fiveMinutes, display);
  }

  $('.custom-expand').attr('aria-expanded', 'true');

  $('.scroll-up').click(function(){
    $('html, body').animate({
        scrollTop: $( "#focus-product-section").offset().top
    }, 2000);
  });

  $('.product-meta__reviews-badge').on('click', function (e) {
      // console.log('scroll');
      e.preventDefault();
      $('html, body').animate({
          scrollTop: $('span#review_top').offset().top
      }, 2000)
  
  });
  $('.variant-swatch').on('click', function () {
      var id = $(this).data('id');
      $('#product__media_' + id).removeClass('hide').addClass('show');
      $('#product__media_' + id).siblings().addClass('hide').removeClass('show');
  });
  
  if (($(window).width()) < 481) {
      $('.image-text-images').slick({
          infinite: false,
          slidesToShow: 1,
          slidesToScroll: 1,
          prevArrow: '<button class="slide-arrow prev-arrow"><img src="' + arrow + '"></button>',
          nextArrow: '<button class="slide-arrow next-arrow"><img src="' + arrow + '"></button>',
          arrows: true
      })
  };

  $('.featured-media_1').slick({
        slidesToShow: 1,
        slidesToScroll: 1,
        arrows: true,
        asNavFor: '.thumb-media_1',
        infinite:true,
    });
    $('.thumb-media_1').slick({
        slidesToShow: 7,
        slidesToScroll: 1,
        asNavFor: '.featured-media_1',
        dots: false,
        centerMode: true,
        focusOnSelect: true,
        infinite: true,
	...
	$(".prev-btn").click(function () {
		$(".featured-media_1").slick("slickPrev");
	});

	$(".next-btn").click(function () {
		$(".featured-media_1").slick("slickNext");
	});
    
	$(".prev-btn").addClass("slick-disabled");
	$(".featured-media_1").on("afterChange", function () {
    ...
    $('.featured-media_2').slick({
        slidesToShow: 1,
        slidesToScroll: 1,
        arrows: false,
        asNavFor: '.thumb-media_2',
        infinite:true,
        // centerMode: false,
        dots: false,
        swipeToSlide: true,
	...
	$(".prev-btn2").click(function () {
		$(".featured-media_2").slick("slickPrev");
	});

	$(".next-btn2").click(function () {
		$(".featured-media_2").slick("slickNext");
	});
    
	$(".prev-btn2").addClass("slick-disabled");
	$(".featured-media_2").on("afterChange", function () {
		if ($(".slick-prev").hasClass("slick-disabled")) {
			$(".prev-btn2").addClass("slick-disabled");
		} else {
...
$(window).scroll(function (event) {
  ...
  if (scroll > screen_H){
    $('.product-sticky--form .product-sticky-form').removeAttr('hidden');
  } else {
    $('.product-sticky--form .product-sticky-form').attr('hidden', 'true');
  }
});
